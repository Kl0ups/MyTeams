...
int handle_display_command6(char *command, transfer_t dt)
{
    if (strcmp(command, "THREADS")) {
        for (int i = 0; i < dt.p_size; i++) {
            client_channel_print_threads(dt.p_data[i].thread.id
            , dt.p_extra.client.id, dt.p_data[i].thread.ctt
    ...
    if (strcmp(command, "REPLIES")) {
        for (int i = 0; i < dt.p_size; i++) {
            client_thread_print_replies(dt.p_extra.thread.id
            , dt.p_data[i].message.t_client, dt.p_data[i].message.ctt
...
int handle_display_command5(char *command, transfer_t dt)
{
    if (strcmp(command, "THREAD CREATED")) {
        return client_print_thread_created(dt.p_data[0].thread.id
    ...
    if (strcmp(command, "REPLY CREATED")) {
        return client_print_reply_created(dt.p_data[0].message.id
        , dt.p_data[0].message.t_client, dt.p_data[0].message.ctt
        , dt.p_data[0].message.msg);
    }
    if (strcmp(command, "SUBSCRIBED")) {
        return client_print_subscribed(dt.p_data[0].client.id
        , dt.p_extra.team.id);
    }
    if (strcmp(command, "UNSUBSCRIBED")) {
        return client_print_unsubscribed(dt.p_data[0].client.id
        , dt.p_extra.team.id);
    }
...
int handle_display_command4(char *command, transfer_t dt)
{
    if (strcmp(command, "CHANNEL")) {
        return client_print_channel(dt.p_data[0].channel.id
        , dt.p_data[0].channel.name, dt.p_data[0].channel.description);
    }
    if (strcmp(command, "TEAM CREATED")) {
        return client_print_team_created(dt.p_data[0].team.id
        , dt.p_data[0].team.name, dt.p_data[0].team.description);
    }
    if (strcmp(command, "CHANNEL CREATED")) {
        return client_print_channel_created(dt.p_data[0].channel.id
        , dt.p_data[0].channel.name, dt.p_data[0].channel.description);
    }
    if (strcmp(command, "CHANNELS")) {
        for (int i = 0; i < dt.p_size; i++) {
            client_team_print_channels(dt.p_data[i].channel.id
...
int handle_display_command3(char *command, transfer_t dt)
{
    if (strcmp(command, "MESSAGES")) {
        for (int i = 0; i < dt.p_size; i++) {
            client_private_message_print_messages(dt.p_data[i].message.t_client
            , dt.p_data[i].message.ctt, dt.p_data[i].message.msg);
        }
        return 0;
    }
    if (strcmp(command, "ERROR TEAM"))
        return client_error_unknown_team(dt.p_data[0].team.id);
    if (strcmp(command, "ERROR CHANNEL"))
        return client_error_unknown_channel(dt.p_data[0].channel.id);
    if (strcmp(command, "ERROR THREAD"))
        return client_error_unknown_thread(dt.p_data[0].thread.id);
    if (strcmp(command, "ERROR USER"))
        return client_error_unknown_user(dt.p_data[0].client.id);
    if (strcmp(command, "TEAM")) {
        return client_print_team(dt.p_data[0].team.id
        , dt.p_data[0].team.name, dt.p_data[0].team.description);
    }
...
int handle_display_command2(char *command, transfer_t dt)
{
    if (strcmp(command, "USERS")) {
        for (int i = 0; i < dt.p_size; i++) {
            client_print_users(dt.p_data[i].client.id
    ...
    if (strcmp(command, "TEAMS")){
        for (int i = 0; i < dt.p_size; i++) {
            client_print_teams(dt.p_data[i].team.id
            , dt.p_data[i].team.name, dt.p_data[i].team.description);
        }
        return 0;
    }
    if (strcmp(command, "THREAD")) {
        return client_print_thread(dt.p_data[0].thread.id
        , dt.p_extra.client.id, dt.p_data[0].thread.ctt
        , dt.p_data[0].thread.name, dt.p_data[0].thread.post);
    }
